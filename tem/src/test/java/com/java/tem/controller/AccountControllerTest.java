 ...
 */
  ...
  void processRegisterEmptyDenominazione() throws Exception {
    ...
    assertTrue(bindingResult.getFieldError("denominazione").toString().contains(sizeErrorString),
			"");
  ...
  void processRegisterUserAlreadyExists() throws Exception {
	    String url = "/process_register";
	    when(accountService.checkUserExistanceByEmail(Mockito.anyString())).thenReturn(Boolean.TRUE);
  ...
  void processRegisterOk() throws Exception {
	    String url = "/process_register";
	    when(accountService.checkUserExistanceByEmail(Mockito.anyString())).thenReturn(Boolean.FALSE);
  ...
  void testUsersPageNoAdmin() throws Exception {
	  ...
	  mockMvc.perform(get(urlString)).andExpect(status().isForbidden()).andExpect(result -> assertTrue(result.getResolvedException() instanceof NotAuthorizedException));
  }
